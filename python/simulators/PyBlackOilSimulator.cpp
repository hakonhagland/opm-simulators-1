...
{
    // this is a proxy object which only purpose is to provide a shared pointer
    // that can be used to produce weak pointers to our self. In this way
    // Python objects generated by the API can check if the simulator is still
    // alive before accessing the simulator pointer..
    simulatorWrapper_ = std::make_shared<PySimulatorWrapper>(*this);
}
...
{
    return new PySchedule{simulatorWrapper_};
}

const Opm::Schedule& PyBlackOilSimulator::getOpmSchedule() {
    return getFlowMainEbos().schedule();
}
