...
void
AdaptiveTimeStepping<TypeTag>::SubStepIteration<Solver>::
chopTimeStep_(const int new_time_step)
{
...
template<class TypeTag>
template<class Solver>
std::pair<SimulatorReportSingle, std::string>
AdaptiveTimeStepping<TypeTag>::SubStepIteration<Solver>::
runSubStep_()
{
    ...
    auto handleFailure = [this, &substep_report, &cause_of_failure]
                         (const std::string& failure_reason, const std::exception& e)
    {
        substep_report = solver_().failureReport();
        cause_of_failure = failure_reason;
        if (solverVerbose_()) {
            std::string message;
    ...
    return {substep_report, cause_of_failure};
}
...
writeOutput_() const
{
    ...
    problem.writeOutput(simulatorTimer_());
