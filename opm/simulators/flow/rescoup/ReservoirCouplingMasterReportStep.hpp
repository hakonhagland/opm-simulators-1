...
public:
    ...
    void getMasterGroupNamesForSlave(unsigned int index) const {
         return this->master_.getMasterGroupNamesForSlave(index);
    }
...
private:
    ...
    // Whether a slave group is a producer or an injector. The keys are the corresponding master group names.
    // The order of the groups in the vector is given by getMasterGroupCanonicalIdx().
    std::map<std::string, std::vector<std::uint8_t>> slave_group_is_producer_;
    std::map<std::string, std::vector<std::uint8_t>> slave_group_is_injector_;
};
} // namespace Opm
