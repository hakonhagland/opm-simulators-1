...
void
ReservoirCouplingTimeStepper<Scalar>::
sendStartOfReportStepFlagToSlaves(bool start_of_report_step)
{
    if (this->comm_.rank() == 0) {
        for (unsigned int slave_idx = 0; slave_idx < this->numSlaves(); slave_idx++) {
            if (!this->slaveIsActivated(slave_idx)) {
                this->logger().info(fmt::format(
                    "Slave {} has not activated yet, skipping sending start of report step flag to slave",
                    this->slaveName(slave_idx)
                ));
                continue;
            }
            // Send a boolean flag to indicate if the timestep is the start of a report step
            // NOTE: See comment about error handling at the top of this file.
            std::uint8_t start_of_report_step_flag = start_of_report_step ? 1u : 0u;
            ...
            this->logger().info(fmt::format(
                "Sent start of report step flag {} from master process rank 0 to slave process "
                "rank 0 with name: {}", start_of_report_step_flag, this->slaveName(slave_idx))
            );
        }
    }
}
