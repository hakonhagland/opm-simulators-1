...
Scalar
ReservoirCouplingMaster<Scalar>::
getMasterGroupInjectionRate(const std::string &group_name, const int phase_pos, const bool res_rates) const
{
    if (res_rates) {
        return this->report_step_data_->getMasterGroupInjectionReservoirRate(group_name, phase_pos);
    }
    else {
        return this->report_step_data_->getMasterGroupInjectionSurfaceRate(group_name, phase_pos);
    }
}
...
{
    if (res_rates) {
        return this->report_step_data_->getMasterGroupProductionReservoirRate(group_name, phase_pos);
    }
    else {
        return this->report_step_data_->getMasterGroupProductionSurfaceRate(group_name, phase_pos);
    }
}
