...
{
    // First check if the unique_ptr is not already initialized
    if (!this->report_step_data_) {
...
void
ReservoirCouplingMaster<Scalar>::
receiveNextReportDateFromSlaves(double current_time)
{
    if (!this->time_stepper_) {
        this->time_stepper_ = std::make_unique<ReservoirCouplingTimeStepper<Scalar>>(*this);
    }
    this->time_stepper_->receiveNextReportDateFromSlaves(current_time);
}
...
receiveNextReportDateFromSlaves()
{
    ...
    this->report_step_data_->receiveNextReportDateFromSlaves();
}
