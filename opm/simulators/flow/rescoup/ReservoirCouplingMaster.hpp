...
public:
    ...
    void maybeReceiveGroupInfoFromSlaves() { this->report_step_data_->maybeReceiveGroupInfoFromSlaves(); }
    ...
    void sendGroupInfoToSlaves(int report_step_idx) {
        this->report_step_data_->sendGroupInfoToSlaves(report_step_idx);
    }
    void sendNextTimeStepToSlaves(double next_time_step) {
        this->time_stepper_->sendNextTimeStepToSlaves(next_time_step);
    }
    void sendStartOfReportStepFlagToSlaves(bool start_of_report_step) {
        this->time_stepper_->sendStartOfReportStepFlagToSlaves(start_of_report_step);
    }
    void setDeferredLogger(DeferredLogger *deferred_logger) {
         this->logger_.setDeferredLogger(deferred_logger);
    }
p    // These are currently only used for unit testing
    ...
    bool slaveGroupIsProducer(int slave_idx, int master_group_idx) const {
        return this->slave_group_is_producer_.at(this->slave_names_[slave_idx])[master_group_idx];
    }
    bool slaveGroupIsInjector(int slave_idx, int master_group_idx) const {
        return this->slave_group_is_injector_.at(this->slave_names_[slave_idx])[master_group_idx];
    }
