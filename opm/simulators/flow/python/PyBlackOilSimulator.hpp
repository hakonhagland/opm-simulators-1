...
public:
    ...
    const Opm::Schedule& getOpmSchedule();
...
private:
    ...
    std::shared_ptr<PySimulatorWrapper> simulatorWrapper_;
};

class PySimulatorWrapper {
public:
    PySimulatorWrapper(PyBlackOilSimulator& sim) : sim_(sim)
    {
    }

    PyBlackOilSimulator& getSimulator() {
        return this->sim_;
    }

private:
    PyBlackOilSimulator& sim_;
};
