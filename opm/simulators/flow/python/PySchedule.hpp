...
class PySchedule {
public:
    PySchedule(std::weak_ptr<PySimulatorWrapper> simulator_wrapper)
        : simulator_wrapper_(simulator_wrapper)
    {
    }
    PyWell *getWell(const std::string& name, const size_t& timestep );
    void shutWell(const std::string& name, const size_t& timestep );
