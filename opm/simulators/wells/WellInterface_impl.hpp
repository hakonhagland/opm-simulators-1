...
namespace Opm
{
                          ...
                          DeferredLogger& deferred_logger)
    {
        ...
        if (!this->wellHasTHPConstraints(summary_state)) {
            const std::string msg = fmt::format("XXX: Well {} does not have THP constraints", well_name);
            deferred_logger.info(msg);
            return;
        }
        ...
        if (!glo.has_well(well_name)) {
            const std::string msg = fmt::format("XXX: Well {} : Gas Lift not activated: "
        ...
        const std::string msg = fmt::format("XXX: Well {} : Setting ALQ to max value: {}",
            well_name, max_alq);
        deferred_logger.info(msg);
    }
