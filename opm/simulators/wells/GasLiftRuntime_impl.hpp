 ...
 *   rate.
 *
 *   NOTE: Currently, this optimization is called from beginTimeStep()
 *     in BlackoilWellModel (which calls computeWellPotentials()). The
 *     well controls are updated later in the time step from
 *     beginIteration() in BlackoilWellModel (which calls assemble() and
 *     then updateWellControls()). If reducing or adding lift-gas further would cause
