...
TargetCalculator(const WellGroupHelperType& wg_helper,
                 ...
                 const Group& group)
    : cmode_{wg_helper.groupState().production_control(group.name())}
...
{
    const auto& pu = this->wg_helper_.phaseUsageInfo();
    switch (this->cmode_) {
...
{
    return this->wg_helper_.getProductionGroupTarget(this->group_, deferred_logger);
}
...
InjectionTargetCalculator(const WellGroupHelperType& wg_helper,
    ...
    , cmode_{wg_helper.groupState().injection_control(group.name(), injection_phase)}
{
    pos_ = this->wg_helper_.phaseToActivePhaseIdx(injection_phase);
...
groupTarget(DeferredLogger& deferred_logger) const
{
    return this->wg_helper_.getInjectionGroupTarget(
