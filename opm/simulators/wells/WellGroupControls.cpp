                            ...
                            DeferredLogger& deferred_logger) const
{
    ...
    auto currentGroupControl = wgHelper.groupState().injection_control(group.name(), injectionPhase);
    if (currentGroupControl == Group::InjectionCMode::FLD ||
        currentGroupControl == Group::InjectionCMode::NONE) {
        if (!group.injectionGroupControlAvailable(injectionPhase)) {
            // We cannot go any further up the hierarchy. This could
            ...
            // Inject share of parents control
            const auto& parent = wgHelper.schedule().getGroup( group.parent(), well_.currentStep());
            efficiencyFactor *= group.getGroupEfficiencyFactor();
            return getGroupInjectionTargetRate(parent, wgHelper, injectorType,
    ...
    return wgHelper.wellState().well(well_.indexOfWell()).group_target;
}
                          ...
                          DeferredLogger& deferred_logger) const
{
    ...
    const auto& schedule = wgHelper.schedule();
    ...
    if (currentGroupControl == Group::ProductionCMode::FLD ||
        ...
        if (!group.productionGroupControlAvailable()) {
            ...
            const auto& parent = schedule.getGroup(group.parent(), well_.currentStep());
            efficiencyFactor *= group.getGroupEfficiencyFactor();
            getGroupProductionControl(parent, wgHelper, bhp,
                                      rates, rateConverter,
    ...
    WGHelpers::TargetCalculator<Scalar, IndexTraits> tcalc{wgHelper, resv_coeff, group};
                             ...
                             DeferredLogger& deferred_logger) const
{
    ...
    if (currentGroupControl == Group::ProductionCMode::FLD ||
        ...
        if (!group.productionGroupControlAvailable()) {
            ...
            const auto& parent = schedule.getGroup(group.parent(), well_.currentStep());
            efficiencyFactor *= group.getGroupEfficiencyFactor();
            return getGroupProductionTargetRate(parent, wgHelper,
    ...
    WGHelpers::TargetCalculator<Scalar, IndexTraits> tcalc{wgHelper,
                                                           resv_coeff,
                                                           group};
                                      ...
                                      DeferredLogger& deferred_logger)
{
    ...
    if (currentGroupControl == Group::ProductionCMode::FLD ||
        ...
        if (!group.productionGroupControlAvailable()) {
            ...
            efficiencyFactor *= group.getGroupEfficiencyFactor();
            return getAutoChokeGroupProductionTargetRate(bottom_group, parent, wgHelper,
    ...
    const auto chain = wgHelper.groupChainTopBot(bottom_group.name(), group.name());
...
#define INSTANTIATE(T,...)                                               \
    ...
    template void WellGroupControls<T, BlackOilDefaultFluidSystemIndices>::                                 \
        getGroupProductionControl(const Group&,                          \
                                  const WellGroupHelper<T, BlackOilDefaultFluidSystemIndices>&,            \
                                  const __VA_ARGS__& bhp,                \
