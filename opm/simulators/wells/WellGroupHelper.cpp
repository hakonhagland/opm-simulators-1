...
accumulateGroupEfficiencyFactor(const Group& group, Scalar& factor)
{
    ...
    if (group.parent() != "FIELD" && !group.parent().empty())
        this->accumulateGroupEfficiencyFactor(
            this->schedule_.getGroup(group.parent(), this->report_step_), factor);
}
                         ...
                         const bool check_guide_rate)
{
        ...
        current_group_control == Group::InjectionCMode::NONE)
    {
        ...
        return this->checkGroupConstraintsInj(
            name,
            parent,
            parent_group,
            rates,
    ...
    if (this->schedule_[this->report_step_].gconsale().has(group.name())) {
        const auto& gconsale = this->schedule_[this->report_step_].gconsale().get(group.name(), this->summary_state_);
        sales_target = gconsale.sales_target;
    }
    WGHelpers::InjectionTargetCalculator<Scalar, IndexTraits> tcalc{
    ...
    WGHelpers::FractionCalculator fcalc{
        this->schedule_,
        this->wellState(),
        this->groupState(),
        this->summary_state_,
    ...
    const auto chain = this->groupChainTopBot_(name, group.name());
    ...
    for (std::size_t ii = 1; ii < num_ancestors; ++ii) {
        const int num_gr_ctrl = this->groupControlledWells_(
            chain[ii],
    ...
    if (this->schedule_.hasWell(name) && this->wellState().well(name).group_target) {  // for wells we already have computed the target
        Scalar scale = 1.0;
    ...
    const Scalar orig_target = tcalc.groupTarget(ctrl, this->deferredLogger());
                          ...
                          const bool check_guide_rate)
{
    ...
    if (current_group_control == Group::ProductionCMode::FLD || current_group_control == Group::ProductionCMode::NONE) {
        // Return if we are not available for parent group.
    ...
    WGHelpers::FractionCalculator<Scalar, IndexTraits> fcalc{
        this->schedule_,
        this->wellState(),
        this->groupState(),
        this->summary_state_,
    ...
    {
        const std::vector<Scalar>& group_surface_rates =
            this->groupState().production_rates(group_name);
        return tcalc.calcModeRateFromRates(group_surface_rates);
    };
    ...
    if (check_guide_rate) {
        for (std::size_t ii = 1; ii < num_ancestors; ++ii) {
            if (this->guide_rate_.has(chain[ii])) {
                ...
                const Scalar grefficiency =
                    this->schedule_.getGroup(guided_group, this->report_step_).getGroupEfficiencyFactor();
                const Scalar current_rate_fraction =
                    grefficiency * local_current_rate_lambda(guided_group) /
                        (local_current_rate_lambda(chain[ii-1]));
                const Scalar guiderate_fraction = local_fraction_lambda(guided_group);
                // we add a factor here to avoid switching due to numerical instability
                const Scalar factor = 1.01;
        ...
        for (const auto& node : leaf_nodes) {
            ...
            if (network.node(node).add_gas_lift_gas()) {
                ...
                for (const std::string& wellname : group.wells()) {
                    ...
                    if (well.isInjector() || !this->wellState().isOpen(wellname)) continue;
                    ...
                    if (well_index.has_value() && this->wellState().wellIsOwned(well_index.value(), wellname)) {
...
WellGroupHelper<Scalar, IndexTraits>::
getProductionGroupRateVector(const std::string& group_name) const
{
                           ...
                           const std::vector<Scalar>& resv_coeff)
{
    ...
    // of the child of group.
    auto current_group_control = this->groupState().injection_control(group.name(), injection_phase);
    if (current_group_control == Group::InjectionCMode::FLD || current_group_control == Group::InjectionCMode::NONE) {
        // Return if we are not available for parent group.
        if (!group.injectionGroupControlAvailable(injection_phase)) {
...
GuideRate::RateVector
WellGroupHelper<Scalar, IndexTraits>::
getWellRateVector(const std::string& name) const
{
...
template<typename Scalar, typename IndexTraits>
void WellGroupHelper<Scalar, IndexTraits>::
setDefaultWellState(WellStateType well_state_type) {
...
sumSolventRates(const Group& group, const bool is_injector)
{
    ...
    for (const std::string& well_name : group.wells()) {
        ...
        const Scalar factor = well_ecl.getEfficiencyFactor() *
                              this->wellState()[well_ecl.name()].efficiency_scaling_factor;
        if (is_injector)
...
template<typename Scalar, typename IndexTraits>
Scalar WellGroupHelper<Scalar, IndexTraits>::
    ...
    if (this->wellState().isRank0() && (group.hasSatelliteProduction() || group.hasSatelliteInjection())) {
        if (injector) {
            rate = this->satelliteInjectionRate_(
                this->schedule_[this->report_step_],
                group,
                phase_pos,
                res_rates
            );
...
Scalar
WellGroupHelper<Scalar, IndexTraits>::
sumWellResRates(const Group& group, const int phasePos, const bool injector) const
{
...
updateNetworkLeafNodeProductionRates()
{
    ...
    if (network.active()) {
        ...
        for (const auto& group_name : network.leaf_nodes()) {
            ...
            if (this->schedule_[this->report_step_].groups.has(group_name)) { // Allow empty leaf nodes that are not groups
                   ...
                   const Parallel::Communication& comm) const
{
    ...
    for (const std::string& child_well : group.wells()) {
        ...
        if (well_index.has_value() && this->wellState().wellIsOwned(well_index.value(), child_well))
        {
            ...
            // Cannot use deferredLogger() accessor here since it's non-const and this method is const.
            // Direct pointer dereference is safe as the logger object itself is mutable.
            auto& deferred_logger = *this->deferred_logger_;
            switch (offended_control){
                ...
                case Group::ProductionCMode::PRBL:
                    OPM_DEFLOG_THROW(std::runtime_error,
                                     "Group " + group.name() +
                                     " GroupProductionCMode PRBL not implemented",
                                     deferred_logger);
                    break;
                case Group::ProductionCMode::CRAT:
                    OPM_DEFLOG_THROW(std::runtime_error,
                                     "Group " + group.name() +
                                     " GroupProductionCMode CRAT not implemented",
                                     deferred_logger);
                    break;
            }
...
Scalar
WellGroupHelper<Scalar, IndexTraits>::
getGuideRate_(const std::string& name, const GuideRateModel::Target target)
{
    if (this->schedule_.hasWell(name, this->report_step_)) {
        if (this->guide_rate_.has(name) || this->guide_rate_.hasPotentials(name)) {
            return this->guide_rate_.get(name, target, this->getWellRateVector_(name));
        } else {
            return 0.0;
        }
    }
    ...
    for (const std::string& group_name : group.groups()) {
        ...
        if (current_group_control == Group::ProductionCMode::FLD
            ...
            total_guide_rate += this->getGuideRate_(group_name, target);
        }
    }
    ...
    for (const std::string& well_name : group.wells()) {
        ...
        total_guide_rate += this->getGuideRate_(well_name, target);
...
WellGroupHelper<Scalar, IndexTraits>::
getGuideRateVector_(const std::vector<Scalar>& rates) const
{
    const auto& pu = this->phase_usage_info_;
    Scalar oilRate = 0.0;
...
GuideRate::RateVector
WellGroupHelper<Scalar, IndexTraits>::
getWellRateVector_(const std::string& name)
{
    return this->getGuideRateVector_(this->wellState().currentWellRates(name));
}
...
WellGroupHelper<Scalar, IndexTraits>::
groupChainTopBot_(const std::string& bottom, const std::string& top)
{
...
template<typename Scalar, typename IndexTraits>
int WellGroupHelper<Scalar, IndexTraits>::
groupControlledWells_(const std::string& group_name,
                     const std::string& always_included_child,
                     const bool is_production_group,
                     const Phase injection_phase)
{
    auto num_wells = is_production_group ? this->groupState().number_of_wells_under_group_control(group_name)
    ...
    if (this->schedule_.hasWell(always_included_child, this->report_step_)) {
        ...
        const bool already_included = is_production_group ? this->wellState().isProductionGrup(always_included_child)
...
bool
WellGroupHelper<Scalar, IndexTraits>::
isInGroupChainTopBot_(const std::string& bottom, const std::string& top)
{
    // Get initial parent, 'bottom' can be a well or a group.
    std::string parent;
    if (this->schedule_.hasWell(bottom, this->report_step_)) {
...
template<typename Scalar, typename IndexTraits>
Scalar WellGroupHelper<Scalar, IndexTraits>::
satelliteInjectionRate_(const ScheduleState& sched,
                       const Group& group,
                       const int phase_pos,
...
WellGroupHelper<Scalar, IndexTraits>::
selectRateComponent_(const int phase_pos) const
{
    // TODO: this function can be wrong, phase_pos is not used anymore, this function requries checking and refactoring.
    const auto& pu = this->phase_usage_info_;
    using Rate = GSatProd::GSatProdGroupProp::Rate;

    for (const auto& [phase, rate_comp] : std::array {
    ...
    for (const std::string& child_well : group.wells()) {
        ...
        if (group.as_choke() && ((ctrl1 == Group::ProductionCMode::FLD) ||
            ...
            if (group_guide_rate > 0) {
                ...
                const auto& control_group_guide_rate = this->getGuideRate_(
                    control_group_name, tcalc.guideTargetMode()
                );
        ...
        if (is_injector) {
            ...
            for (Phase phase : all) {
                ...
                const int num_group_controlled_wells
                        = this->groupControlledWells_(sub_group.name(), "", !is_injector, phase);
                if (individual_control || num_group_controlled_wells == 0) {
                    ...
                    if (!this->guide_rate_.has(sub_group.name(), phase)) {
                        group_target_reduction[phase_pos] += sub_group_efficiency *
                                                               sub_group_target_reduction[phase_pos];
                    }
                }
            }
        }
            ...
            if (individual_control || num_group_controlled_wells == 0) {
                ...
                // The subgroup may participate in group control.
                    ...
                    // Accumulate from this subgroup only if no group guide rate is set for it.
                    for (int phase = 0; phase < np; phase++) {
                        group_target_reduction[phase] += sub_group_efficiency *
                                                    sub_group_target_reduction[phase];
                    }
                }
            }
        }
