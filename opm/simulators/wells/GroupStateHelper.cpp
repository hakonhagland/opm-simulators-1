...
getGroupRatesAvailableForHigherLevelControl(const Group& group, const bool is_injector) const
{
    ...
    if (is_injector) {
        ...
        for (int phasePos = 0; phasePos < this->numPhases(); ++phasePos) {
            rates[phasePos] =
                this->sumWellSurfaceRates(group, phasePos, /*is_injector=*/true) - reduction_rates[phasePos];
        }
    }
    else {
        ...
        for (int phasePos = 0; phasePos < this->numPhases(); ++phasePos) {
            rates[phasePos] =
                this->sumWellSurfaceRates(group, phasePos, /*is_injector=*/false) - reduction_rates[phasePos];
        }
    }
}
