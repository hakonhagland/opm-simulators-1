...
namespace Opm
{
                    ...
                    DeferredLogger& deferred_logger) const
    {
        ...
        std::vector<Value> b_perfcells_dense(this->numConservationQuantities(), zeroElem());
        ...
        std::vector<Value> cmix_s(this->numConservationQuantities(), zeroElem());
                                   ...
                                   DeferredLogger& deferred_logger)
    {
        ...
        assembleWellEqWithoutIterationImpl(simulator, dt, inj_controls,
                                           prod_controls, wgHelper, well_state,
                                           deferred_logger);
    }
                                       ...
                                       DeferredLogger& deferred_logger)
    {
        ...
        for (int perf = 0; perf < this->number_of_local_perforations_; ++perf) {
            ...
            for (int componentIdx = 0; componentIdx < this->num_conservation_quantities_; ++componentIdx) {
                        ...
                        DeferredLogger& deferred_logger) const
    {
        ...
        if (FluidSystem::phaseUsage().hasCO2orH2Store()) {
        ...
        if (FluidSystem::phaseIsActive(FluidSystem::waterPhaseIdx)) {
            const unsigned wat_comp_idx = FluidSystem::canonicalToActiveCompIdx(FluidSystem::waterCompIdx);
    ...
    void
    StandardWell<TypeTag>::
    updateWellStateFromPrimaryVariables(WellStateType& well_state,
    ...
    {
        const auto& summaryState = simulator.vanguard().summaryState();
        const auto obtain_bhp = this->isProducer() ? computeBhpAtThpLimitProd(well_state, simulator, wgHelper, summaryState, deferred_logger)
        : computeBhpAtThpLimitInj(simulator, wgHelper, summaryState, deferred_logger);
                       ...
                       const bool relax_tolerance) const
    {
        ...
        ConvergenceReport report = this->StdWellEval::getWellConvergence(wgHelper.wellState(),
                                                                         B_avg,
                                   ...
                                   DeferredLogger& deferred_logger)
    {
         ...
         computeWellConnectionDensitesPressures(simulator, wgHelper,
                              ...
                              DeferredLogger& deferred_logger)
    {
        ...
        updateWellState(simulator, dx_well, wgHelper, well_state, deferred_logger);
    }
                                          ...
                                          DeferredLogger& deferred_logger)
    {
        ...
        updateWellState(simulator, xw, wgHelper, well_state, deferred_logger);
    }
                                      ...
                                      DeferredLogger& deferred_logger) const
    {
        ...
        // Ensure that wgHelper_copy uses well_state_copy as WellState for the rest of this function,
                               ...
                               const WellStateType& well_state) const
    {
        ...
        if (well.isInjector()){
            ...
            auto bhp_at_thp_limit = computeBhpAtThpLimitInj(simulator, wgHelper, summary_state, deferred_logger);
            if (bhp_at_thp_limit) {
            ...
            computeWellRatesWithThpAlqProd(
                simulator, wgHelper, summary_state,
                                  ...
                                  DeferredLogger& deferred_logger) const
    {
       ...
       // update connection pressures relative to updated bhp to get better estimate of connection dp
        ...
        well_copy.calculateExplicitQuantities(simulator, wgHelper_copy, deferred_logger);
        const double dt = simulator.timeStepSize();
        ...
        for (int comp_idx = 0; comp_idx < this->num_conservation_quantities_; ++comp_idx) {
            ...
            well_potentials[FluidSystem::activeCompToActivePhaseIdx(comp_idx)] = rate.value();
        }
                               ...
                               Scalar alq) const
    {
        ...
        auto bhp_at_thp_limit = computeBhpAtThpLimitProdWithAlq(
                              simulator, wgHelper, summary_state, alq, deferred_logger, /*iterate_if_no_solution */ true);
        if (bhp_at_thp_limit) {
                          ...
                          DeferredLogger& deferred_logger) // const
    {
        ...
        if (!converged_implicit) {
            ...
            if (!Base::wellHasTHPConstraints(summaryState) || bhp_controlled_well) {
                ...
                computeWellRatesWithBhpIterations(simulator, bhp, wgHelper, well_potentials, deferred_logger);
            } else {
                // the well has a THP related constraint
                well_potentials = computeWellPotentialWithTHP(simulator, wgHelper, deferred_logger, well_state);
            }
        }
                                    ...
                                    bool iterate_if_no_solution) const
    {
        ...
        auto fratesIter = [this, &simulator, &wgHelper, &deferred_logger](const Scalar bhp) {
            // Solver the well iterations to see if we are
            // able to get a solution with an update
            // solution
            std::vector<Scalar> rates(3);
            computeWellRatesWithBhpIterations(simulator, bhp, wgHelper, rates, deferred_logger);
            this->adaptRatesForVFP(rates);
            return rates;
        };
    ...
    computeBhpAtThpLimitInj(const Simulator& simulator,
                            ...
                            DeferredLogger& deferred_logger) const
    {
        // Note: wgHelper parameter is currently unused in StandardWell but kept for consistency
                             ...
                             DeferredLogger& deferred_logger)
    {
        ...
        do {
            ...
            ++it;
            solveEqAndUpdateWellState(simulator, wgHelper, well_state, deferred_logger);
                            ...
                            DeferredLogger& deferred_logger) const
    {
        ...
        std::vector<Scalar> well_q_s(this->num_conservation_quantities_, 0.);
        const EvalWell& bhp = this->primary_variables_.eval(Bhp);
        const bool allow_cf = this->getAllowCrossFlow() || openCrossFlowAvoidSingularity(simulator);
        for (int perf = 0; perf < this->number_of_local_perforations_; ++perf) {
            const int cell_idx = this->well_cells_[perf];
