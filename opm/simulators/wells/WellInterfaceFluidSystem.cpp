...
bool
WellInterfaceFluidSystem<FluidSystem>::
zeroGroupRateTarget(const WellGroupHelperType& wgHelper,
                    DeferredLogger& deferred_logger) const
{
    const auto& well_state = wgHelper.wellState();
    const auto& well = this->well_ecl_;
    const auto& group = wgHelper.schedule().getGroup(well.groupName(), this->currentStep());
    ...
    if (this->isInjector()) {
        ...
        const auto& controls = well.injectionControls(wgHelper.summaryState());
        ...
        const Scalar scale =
            this->getGroupProductionTargetRate(group, wgHelper,
                                               efficiencyFactor,
                                               deferred_logger);
        return scale == 0.0;
    }
